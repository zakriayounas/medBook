...
export const getQueryFilters = (req, role) => {
  ...
  const min_fee = parseFloat(searchParams.get("min_fee") || 0);
  const max_fee = searchParams.get("max_fee")
    ? parseFloat(searchParams.get("max_fee"))
    : null;
  ...
  if (min_fee || max_fee) {
    feeQuery = {
      fee: {
        ...(min_fee && { gte: parseFloat(min_fee) }),
        ...(max_fee && { lte: parseFloat(max_fee) }),
      },
    };
  }
  ...
  if (min_rating || max_rating) {
    ratingQuery = {
      ratingAverage: {
        ...(min_rating && { gte: parseFloat(min_rating) }),
        ...(max_rating && { lte: parseFloat(max_rating) }),
      },
    };
  }
