...
const buildAppointmentFilters = (searchParams, userRole, doctorId, userId) => {
  ...
  if (status) {
    ...
    statuses.forEach((s) => {
      if (s === "canceled") {
        whereClause.AND.push({ isCancel: true });
      }

      if (s === "completed") {
        whereClause.AND.push({ isCompleted: true });
      }
...
const buildDoctorFilters = (searchParams) => {
  ...
  const joinedFromDate = searchParams.get("join_from_date");
  const joinedToDate = searchParams.get("join_to_date");
  const consultationType = searchParams.get("consultation_type"); // online / clinic
  ...
  // Consultation type
  if (consultationType) {
    whereClause.consultationType = {
      in: consultationType.split(","),
    };
  }
