...
export const extractLoggedUserDetail = (req) => {
    ...
    const doctorId = req.headers.get("x-doctorId");

    if (!userId || isNaN(userId)) {
        throw new Error("Invalid user ID");
    }

    return { userId: parseInt(userId, 10), userEmail, userRole, doctorId: parseInt(doctorId, 10) }
};
...
export const getQueryFilters = (req, role) => {
    ...
    const isAppointmentRoute = pathname.includes('/api/appointment')
    const { userRole, doctorId, userId } = extractLoggedUserDetail(req)
    const specialties = searchParams.get("specialties");
    const name = searchParams.get("name");
    const gender = searchParams.get("gender");
    const sort_by = searchParams.get("sort_by");
    const doctorIds = userRole === "DOCTOR" ? String(doctorId) : searchParams.get("doctorId");
    const patientIds = userRole === "PATIENT" ? String(userId) : searchParams.get("patientId");
    const page = parseInt(searchParams.get("page") || "1", 10); // Default to 0 if page is not provided
    ...
    // appointment filters
    if (doctorIds && isAppointmentRoute) {
    ...
    if (patientIds && isAppointmentRoute) {
    ...
    return { whereClause, skipRecords, limitRecords, orderBy, userRole, page };
};
