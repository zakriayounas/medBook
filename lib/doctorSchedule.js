...
export const checkDoctorAvailability = async (doctorId, appointmentDateTime) => {
    const doctorSchedules = await getDoctorSchedules(doctorId);

    if (!doctorSchedules.length) {
        return false; // No schedule for the doctor
    }

    // Get the day of the week from the appointment date
    const appointmentDateTimeObj = new Date(appointmentDateTime);
    const appointmentDay = appointmentDateTimeObj.toLocaleString("en-US", {
        weekday: "long",
    });
    ...
    if (!schedule) {
        return false; // No schedule for the doctor on the given day
    }

    // Extract appointment time
    const appointmentHour = appointmentDateTimeObj.getHours();
    const appointmentMinute = appointmentDateTimeObj.getMinutes();

    // Extract hours and minutes from startTime and endTime
    const [startHour, startMinute] = schedule.startTime.split(":").map(Number);
    const [endHour, endMinute] = schedule.endTime.split(":").map(Number);

    // Check if the appointment time falls within the schedule for the day
    ...
    console.log(isWithinSchedule, "schedule")
    if (!isWithinSchedule) {
        return false; // The requested time is outside the doctor's schedule for the day
    }
